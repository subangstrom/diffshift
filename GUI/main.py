...
class ShiftThread(QThread):
    ...
    def __init__(self,microscope, Shifts,rotation0,inner_R,outter_R,is_angularmode,parent=None):
    ...
    def regularshift(self):
        for shiftx,shifty in self.Shifts:
            ...
            self.microscope.set_diffraction_shift(shift)
    ...
    def angular(self):
        for shiftx,shifty in self.Shifts:
            ...
            if xshift_old**2+yshift_old**2 >= self.inner_R**2 and xshift_old**2+yshift_old**2 <= self.outter_R**2:
                self.microscope.set_diffraction_shift(shift)
                self.shift_count += 1
                self.notifyprocess.emit(str(self.shift_count))
                sleep(self.dwelltime) #in seconds
...
class Diffshift(QDialog):
    def __init__(self):
        ...
        self.total_shifts = 0
        self.microscope =  temscript.microscope.Microscope()
        self.detectorsize = 128
        self.overlap = 0.2  #default 20% overlape
        self.shift_startposition = self.microscope.get_diffraction_shift()
        self.shiftthread = ShiftThread(self.microscope,[],self.rotation0,0,0,True)
    ...
    def setupShifts(self):
        ...
        if self.Angular.isChecked() == True:
            Shifts,total_shifts = self.shiftslist(outter_R,outter_R)
        else:
            Shifts,total_shifts = self.shiftslist(Xwidth,Ywidth)
        return Shifts,total_shifts

    def shiftslist(self,Xwidth,Ywidth):
    ...
    def resetshift(self):
        self.microscope.set_diffraction_shift(self.shift_startposition)
        self.Shift.setText('')
